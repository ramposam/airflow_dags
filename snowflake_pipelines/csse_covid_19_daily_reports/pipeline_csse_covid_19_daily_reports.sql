...
CREATE STAGE IF NOT EXISTS MIRROR_DB.MIRROR.STG_CSSE_COVID_19_DAILY_REPORTS_S3
  ...
  -- Do not hardcode AWS credentials. Use: CREDENTIALS=(AWS_KEY_ID=.*
...
SELECT
    'MIRROR_DB.MIRROR.PIPE_CSSE_COVID_19_DAILY_REPORTS' AS pipe_name,
    FILE_NAME,
    ERROR_MESSAGE
FROM TABLE(INFORMATION_SCHEMA.LOAD_HISTORY_BY_PIPE('MIRROR_DB.MIRROR.PIPE_CSSE_COVID_19_DAILY_REPORTS'))
...
CREATE OR REPLACE TASK MIRROR_DB.MIRROR.TASK_CSSE_COVID_19_DAILY_REPORTS
            ...
            MERGE INTO MIRROR_DB.MIRROR.T_ML_CSSE_COVID_19_DAILY_REPORTS AS TARGET
                        USING (
                            SELECT
                                FIPS , ADMIN2 , PROVINCE_STATE , COUNTRY_REGION , LAST_UPDATE , LAT , LONG_ , CONFIRMED , DEATHS , RECOVERED , ACTIVE , COMBINED_KEY , INCIDENT_RATE , CASE_FATALITY_RATIO , FILE_DATE , FILENAME , FILE_ROW_NUMBER , FILE_LAST_MODIFIED , CREATED_DTS , CREATED_BY , NULL as UPDATED_DTS , NULL AS UPDATED_BY , NULL AS UNIQUE_HASH_ID , NULL AS ROW_HASH_ID
                            FROM MIRROR_DB.MIRROR.STREAM_CSSE_COVID_19_DAILY_REPORTS_TR
                        ) AS SOURCE
                        ON TARGET.UNIQUE_HASH_ID = SOURCE.UNIQUE_HASH_ID
                        WHEN MATCHED THEN
                            UPDATE SET
                                TARGET.FIPS = SOURCE.FIPS,TARGET.ADMIN2 = SOURCE.ADMIN2,TARGET.PROVINCE_STATE = SOURCE.PROVINCE_STATE,TARGET.COUNTRY_REGION = SOURCE.COUNTRY_REGION,TARGET.LAST_UPDATE = SOURCE.LAST_UPDATE,TARGET.LAT = SOURCE.LAT,TARGET.LONG_ = SOURCE.LONG_,TARGET.CONFIRMED = SOURCE.CONFIRMED,TARGET.DEATHS = SOURCE.DEATHS,TARGET.RECOVERED = SOURCE.RECOVERED,TARGET.ACTIVE = SOURCE.ACTIVE,TARGET.COMBINED_KEY = SOURCE.COMBINED_KEY,TARGET.INCIDENT_RATE = SOURCE.INCIDENT_RATE,TARGET.CASE_FATALITY_RATIO = SOURCE.CASE_FATALITY_RATIO,TARGET.FILE_DATE = SOURCE.FILE_DATE,TARGET.FILENAME = SOURCE.FILENAME,TARGET.FILE_ROW_NUMBER = SOURCE.FILE_ROW_NUMBER,TARGET.FILE_LAST_MODIFIED = SOURCE.FILE_LAST_MODIFIED,TARGET.CREATED_DTS = SOURCE.CREATED_DTS,TARGET.CREATED_BY = SOURCE.CREATED_BY,TARGET.UPDATED_DTS = SOURCE.UPDATED_DTS,TARGET.UPDATED_BY = SOURCE.UPDATED_BY,TARGET.UNIQUE_HASH_ID = SOURCE.UNIQUE_HASH_ID,TARGET.ROW_HASH_ID = SOURCE.ROW_HASH_ID
                        WHEN NOT MATCHED THEN
                            INSERT (
                                FIPS , ADMIN2 , PROVINCE_STATE , COUNTRY_REGION , LAST_UPDATE , LAT , LONG_ , CONFIRMED , DEATHS , RECOVERED , ACTIVE , COMBINED_KEY , INCIDENT_RATE , CASE_FATALITY_RATIO , FILE_DATE , FILENAME , FILE_ROW_NUMBER , FILE_LAST_MODIFIED , CREATED_DTS , CREATED_BY , UPDATED_DTS , UPDATED_BY , UNIQUE_HASH_ID , ROW_HASH_ID
                            )
                            VALUES (
                                SOURCE.FIPS , SOURCE.ADMIN2 , SOURCE.PROVINCE_STATE , SOURCE.COUNTRY_REGION , SOURCE.LAST_UPDATE , SOURCE.LAT , SOURCE.LONG_ , SOURCE.CONFIRMED , SOURCE.DEATHS , SOURCE.RECOVERED , SOURCE.ACTIVE , SOURCE.COMBINED_KEY , SOURCE.INCIDENT_RATE , SOURCE.CASE_FATALITY_RATIO , SOURCE.FILE_DATE , SOURCE.FILENAME , SOURCE.FILE_ROW_NUMBER , SOURCE.FILE_LAST_MODIFIED , SOURCE.CREATED_DTS , SOURCE.CREATED_BY , SOURCE.UPDATED_DTS , SOURCE.UPDATED_BY , SOURCE.UNIQUE_HASH_ID , SOURCE.ROW_HASH_ID
                                );
...
CREATE OR REPLACE TASK STAGE_DB.STAGE.TASK_CSSE_COVID_19_DAILY_REPORTS
            ...
            AS
            -- you could write merge statement incase you wanted upsert target, src as stream
            MERGE INTO STAGE_DB.STAGE.T_STG_CSSE_COVID_19_DAILY_REPORTS AS TARGET
                        USING (
                            SELECT
                                FIPS , ADMIN2 , PROVINCE_STATE , COUNTRY_REGION , LAST_UPDATE , LAT , LONG_ , CONFIRMED , DEATHS , RECOVERED , ACTIVE , COMBINED_KEY , INCIDENT_RATE , CASE_FATALITY_RATIO , CREATED_DTS , CREATED_BY , UPDATED_DTS , UPDATED_BY , UNIQUE_HASH_ID , ROW_HASH_ID , 'Y' as ACTIVE_FL , FILE_DATE AS EFFECTIVE_START_DATE , '9999-12-31' AS EFFECTIVE_END_DATE
                            FROM STAGE_DB.STAGE.STREAM_CSSE_COVID_19_DAILY_REPORTS
                        ) AS SOURCE
                        ON TARGET.UNIQUE_HASH_ID = SOURCE.UNIQUE_HASH_ID
                        WHEN MATCHED THEN
                            UPDATE SET
                                TARGET.FIPS = SOURCE.FIPS,TARGET.ADMIN2 = SOURCE.ADMIN2,TARGET.PROVINCE_STATE = SOURCE.PROVINCE_STATE,TARGET.COUNTRY_REGION = SOURCE.COUNTRY_REGION,TARGET.LAST_UPDATE = SOURCE.LAST_UPDATE,TARGET.LAT = SOURCE.LAT,TARGET.LONG_ = SOURCE.LONG_,TARGET.CONFIRMED = SOURCE.CONFIRMED,TARGET.DEATHS = SOURCE.DEATHS,TARGET.RECOVERED = SOURCE.RECOVERED,TARGET.ACTIVE = SOURCE.ACTIVE,TARGET.COMBINED_KEY = SOURCE.COMBINED_KEY,TARGET.INCIDENT_RATE = SOURCE.INCIDENT_RATE,TARGET.CASE_FATALITY_RATIO = SOURCE.CASE_FATALITY_RATIO,TARGET.CREATED_DTS = SOURCE.CREATED_DTS,TARGET.CREATED_BY = SOURCE.CREATED_BY,TARGET.UPDATED_DTS = SOURCE.UPDATED_DTS,TARGET.UPDATED_BY = SOURCE.UPDATED_BY,TARGET.UNIQUE_HASH_ID = SOURCE.UNIQUE_HASH_ID,TARGET.ROW_HASH_ID = SOURCE.ROW_HASH_ID,TARGET.ACTIVE_FL = SOURCE.ACTIVE_FL,TARGET.EFFECTIVE_START_DATE = SOURCE.EFFECTIVE_START_DATE,TARGET.EFFECTIVE_END_DATE = SOURCE.EFFECTIVE_END_DATE
                        WHEN NOT MATCHED THEN
                            INSERT (
                                FIPS , ADMIN2 , PROVINCE_STATE , COUNTRY_REGION , LAST_UPDATE , LAT , LONG_ , CONFIRMED , DEATHS , RECOVERED , ACTIVE , COMBINED_KEY , INCIDENT_RATE , CASE_FATALITY_RATIO , CREATED_DTS , CREATED_BY , UPDATED_DTS , UPDATED_BY , UNIQUE_HASH_ID , ROW_HASH_ID , ACTIVE_FL , EFFECTIVE_START_DATE , EFFECTIVE_END_DATE
                            )
                            VALUES (
                                SOURCE.FIPS , SOURCE.ADMIN2 , SOURCE.PROVINCE_STATE , SOURCE.COUNTRY_REGION , SOURCE.LAST_UPDATE , SOURCE.LAT , SOURCE.LONG_ , SOURCE.CONFIRMED , SOURCE.DEATHS , SOURCE.RECOVERED , SOURCE.ACTIVE , SOURCE.COMBINED_KEY , SOURCE.INCIDENT_RATE , SOURCE.CASE_FATALITY_RATIO , SOURCE.CREATED_DTS , SOURCE.CREATED_BY , SOURCE.UPDATED_DTS , SOURCE.UPDATED_BY , SOURCE.UNIQUE_HASH_ID , SOURCE.ROW_HASH_ID , SOURCE.ACTIVE_FL , SOURCE.EFFECTIVE_START_DATE , SOURCE.EFFECTIVE_END_DATE
                                );
