 ...
 CREATE OR REPLACE TABLE META_DB.META.T_FILE_META_DETAILS ( 
        ...
        CREATE TABLE IF NOT EXISTS META_DB.META.T_FILE_VALIDATION_DETAILS (
         ...
         INSERT INTO META_DB.META.T_FILE_META_DETAILS
            SELECT *,PARSE_JSON('[{"TITLE": "STRING"}, {"TYPE": "STRING"}, {"GENRE": "STRING"}, {"RELEASE_YEAR": "STRING"}, {"RATING": "STRING"}, {"DURATION": "STRING"}, {"COUNTRY": "STRING"}]'),current_user(),current_timestamp()  from values
            ('MIRROR_DB','MIRROR','T_ML_NETFLIX_MOVIES_AND_TV_SHOWS_TR','NETFLIX_MOVIES_AND_TV_SHOWS','V1','Y','2021-01-01','9999-12-31');
        
 CREATE DATABASE IF NOT EXISTS MIRROR_DB;
 CREATE SCHEMA IF NOT EXISTS MIRROR;
  CREATE TABLE IF NOT EXISTS MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS_TR (
    TITLE STRING,
    TYPE STRING,
    GENRE STRING,
    RELEASE_YEAR STRING,
 ...
 CREATE DATABASE IF NOT EXISTS MIRROR_DB;
 CREATE SCHEMA IF NOT EXISTS MIRROR;
  CREATE TABLE IF NOT EXISTS MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS (
    TITLE STRING,
    TYPE STRING,
    GENRE STRING,
    RELEASE_YEAR STRING,
 ...
 CREATE SCHEMA IF NOT EXISTS STAGE;
  CREATE TABLE IF NOT EXISTS STAGE_DB.STAGE.T_STG_NETFLIX_MOVIES_AND_TV_SHOWS (
    TITLE STRING,
    TYPE STRING,
    GENRE STRING,
    RELEASE_YEAR NUMBER,
 ...
 CREATE PIPE IF NOT EXISTS  MIRROR_DB.MIRROR.PIPE_NETFLIX_MOVIES_AND_TV_SHOWS
    AUTO_INGEST = TRUE     
AS COPY INTO MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS_TR
        FROM (
            SELECT $1 as TITLE,$2 as TYPE,$3 as GENRE,$4 as RELEASE_YEAR,$5 as RATING,$6 as DURATION,$7 as COUNTRY,$8 as FILE_DATE,metadata$filename as filename,metadata$file_row_number as file_row_number,metadata$file_last_modified as file_last_modified,
            current_timestamp as created_dts, current_user as created_by
            FROM '@MIRROR_DB.MIRROR.STG_NETFLIX_MOVIES_AND_TV_SHOWS_S3'
        )
        PATTERN = '.*\.csv$'
        FILE_FORMAT = (FORMAT_NAME=MIRROR_DB.MIRROR.FF_NETFLIX_MOVIES_AND_TV_SHOWS)
        ON_ERROR = 'CONTINUE'  ;

-- To load all the available files under the path(Historical Data)
ALTER PIPE MIRROR_DB.MIRROR.PIPE_NETFLIX_MOVIES_AND_TV_SHOWS REFRESH;
...
select  SYSTEM$PIPE_STATUS('MIRROR_DB.MIRROR.PIPE_NETFLIX_MOVIES_AND_TV_SHOWS');
...
SELECT 
    'PIPE_NETFLIX_MOVIES_AND_TV_SHOWS' AS pipe_name,
    FILE_NAME,
    ERROR_MESSAGE
FROM TABLE(INFORMATION_SCHEMA.LOAD_HISTORY_BY_PIPE('PIPE_NETFLIX_MOVIES_AND_TV_SHOWS'))
WHERE STATUS = 'LOAD_FAILED';


CREATE OR REPLACE STREAM MIRROR_DB.MIRROR.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS_TR ON TABLE MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS_TR
         append_only = true; 
         
CREATE OR REPLACE TASK META_DB.META.TASK_NETFLIX_MOVIES_AND_TV_SHOWS_TR_VALIDATION
            WAREHOUSE = COMPUTE_WH
            AFTER MIRROR_DB.MIRROR.TASK_LOG_SNOWPIPE_ERRORS            
            WHEN SYSTEM$STREAM_HAS_DATA('MIRROR_DB.MIRROR.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS_TR') -- Skips execution if the stream has no data
            AS
            CALL META_DB.META.VALIDATE_FILE_AND_TABLE(
                'NETFLIX_MOVIES_AND_TV_SHOWS',
                'STG_NETFLIX_MOVIES_AND_TV_SHOWS_S3',
                'MIRROR_DB',
                'MIRROR',
                'T_ML_NETFLIX_MOVIES_AND_TV_SHOWS_TR',
                '',
                1
            );
        
CREATE OR REPLACE TASK MIRROR_DB.MIRROR.TASK_NETFLIX_MOVIES_AND_TV_SHOWS
            SCHEDULE = 'USING CRON 0 23 * * 1-5 UTC'
            WAREHOUSE = 'COMPUTE_WH'
            -- without condition, always try to execute the task
            WHEN
             SYSTEM$STREAM_HAS_DATA('MIRROR_DB.MIRROR.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS_TR') -- skips when stream has no data
            AS
            -- you could write merge statement incase you wanted upsert target, src as stream
            INSERT INTO MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS
            SELECT *  exclude (METADATA$ACTION,METADATA$ISUPDATE,METADATA$ROW_ID)
            FROM MIRROR_DB.MIRROR.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS_TR; 
 ALTER TASK MIRROR_DB.MIRROR.TASK_NETFLIX_MOVIES_AND_TV_SHOWS RESUME;
        
CREATE OR REPLACE STREAM STAGE_DB.STAGE.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS ON TABLE MIRROR_DB.MIRROR.T_ML_NETFLIX_MOVIES_AND_TV_SHOWS
         append_only = true; 
         
CREATE OR REPLACE TASK STAGE_DB.STAGE.TASK_NETFLIX_MOVIES_AND_TV_SHOWS
            SCHEDULE = 'USING CRON 0 23 * * 1-5 UTC'
            WAREHOUSE = 'COMPUTE_WH'
            -- without condition, always try to execute the task
            WHEN
             SYSTEM$STREAM_HAS_DATA('STAGE_DB.STAGE.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS') -- skips when stream has no data
            AS
            -- you could write merge statement incase you wanted upsert target, src as stream
            INSERT INTO STAGE_DB.STAGE.T_STG_NETFLIX_MOVIES_AND_TV_SHOWS
            SELECT *  exclude (METADATA$ACTION,METADATA$ISUPDATE,METADATA$ROW_ID)
            FROM STAGE_DB.STAGE.STREAM_NETFLIX_MOVIES_AND_TV_SHOWS; 
 ALTER TASK STAGE_DB.STAGE.TASK_NETFLIX_MOVIES_AND_TV_SHOWS RESUME;
